...
if true
    ...
    if calcV_flag
        ...
        # for (ilo, zi) in enumerate(range(-R*1.5, R*1.5, Nff))
            ...
            tspan = (0.0, 9 * (0.3 + sin(pi*(ilo-1)/Nff))^0.7)  # Intervalo de "tempo" (parâmetro ao longo da superfície)
            println(tspan)
    ...
    if calcE_flag
        ...
        k = R+0.05
        starts = [
            [k*cos(to), k*sin(to), 0.0] for to in range(0, 2pi-dt, step=dt)
        ]
        append!(starts, [[k*cos(to), 0.0, k*sin(to)] for to in range(0, 2pi-dt, step=dt) if (to!=0) && (abs(to-pi)>1e-4)])
        k = (R+0.05)*cos(pi/4)
        h = (R+0.05)*sin(pi/4)
        append!(starts, [[k*cos(to), k*sin(to), h] for to in range(0, 2pi-dt, step=dt)])
        k = (R+0.05)*cos(-pi/4)
        h = (R+0.05)*sin(-pi/4)
        append!(starts, [[k*cos(to), k*sin(to), h] for to in range(0, 2pi-dt, step=dt)])
        Nd = 3.5
